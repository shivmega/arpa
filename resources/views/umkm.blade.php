            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden lg:px-4">
                <div
                    ...
                    <div class="w-full md:w-1/2 flex">
                        <button type="button" onclick="window.location.href='{{ route('export.pdf.umkm') }}'"
                        ...
                        <button type="button" onclick="window.location.href='{{ route('export.excel.umkm') }}'"
                    ...
                    <div
                        ...
                        <button type="button" onclick="window.location.href='{{ route('halaman.tambah.umkm') }}'"
                        ...
                        @if (Auth::user()->role == 'desa')
                            <button type="button" onclick="window.location.href='{{ route('umkm_desa') }}'"
                                class="flex items-center justify-center text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-4 py-2  focus:outline-none">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                ...
                                Lihat UMKM {{ $desas->find(Auth::user()->desa)->nama_desa }}
                            </button>
                        @endif

                    </div>
                </div>
                <div class="flex flex-col md:flex-row items-center justify-between  md:space-x-2 p-4">
                    <div class="w-full lg:w-1/2">
                        <form class="flex" action="{{route('umkm')}}">
                            ...
                            <div class="relative w-full">
                                <select id="desa" name="desa"
                                    ...
                                    @foreach ($desas as $desa)
                                        <option value="{{$desa->id}}">Desa {{$desa->nama_desa}}, Kecamatan {{$desa->kecamatan}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="submit"
                        ...
                        <tbody>
                            @foreach ($umkms as $umkm)
                                <tr
                                    ...
                                    <td class="px-4 py-3">
                                        @if ($umkm->foto_produk)
                                            <img src="{{ asset('storage/foto-produk/' . $umkm->foto_produk) }}"
                                                width="200">
                                        @else
                                    ...
                                    <td class="px-4 py-3 items-end">
                                        <button type="button"
                                            onclick="window.location.href='{{ route('umkm.details', ['umkm' => $umkm]) }}'"
                                            class="px-1 py-1 mr-3 text-sm font-medium text-center items-center text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 shadow-lg shadow-teal-500/50 dark:shadow-lg dark:shadow-teal-800/80 rounded-lg">
                                        ...
                                        @if (Auth::user()->role == 'super_admin' || (Auth::user()->desa == $umkm->desa && Auth::user()->role == 'desa'))
                                            <form action="{{ route('delete.umkm', ['umkm' => $umkm]) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" value="Delete"
                                                    class="px-1 py-1 text-white mr-3 bg-gradient-to-r text-sm  font-medium text-center items-center from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 rounded-lg">
